...
module tfm_density
  ...
  function tfm_density_salamantin2007(nz, dt, depth, density, temperature, &
    ...
    integer                   :: n
    real(prec)                :: norm_radius_max
    real(prec)                :: transition_dens
    real(prec)                :: average_neck_0
    real(prec), dimension(nz) :: rel_density
    real(prec), dimension(nz) :: norm_radius_i
    real(prec), dimension(nz) :: norm_radius_ii
    real(prec), dimension(nz) :: norm_radius_test
    real(prec), dimension(nz) :: norm_radius_diff
    real(prec), dimension(nz) :: coord_number
    real(prec), dimension(nz) :: fraction_not_involved
    real(prec), dimension(nz) :: average_neck
    real(prec), dimension(nz) :: zeta
    real(prec), dimension(nz) :: spherical_cup
    real(prec), dimension(nz) :: stress
    real(prec), dimension(nz) :: strain_rate
    real(prec), dimension(nz) :: nonlinear_arrhenius
    real(prec), dimension(nz) :: dilatance_rate
    real(prec), dimension(nz) :: rearr_fraction
    real(prec), dimension(nz) :: gbs_arrhenius
    real(prec), dimension(nz) :: gbs_coefficient

    real(prec), parameter :: C = 40.0 
    real(prec), parameter :: Z0 = 6.75
    real(prec), parameter :: ZETA0 = 0.55
    real(prec), parameter :: ALPHA = 3.5
    real(prec), parameter :: QP = 58000.0
    real(prec), parameter :: MUSTAR = 21.0
    real(prec), parameter :: REF_TEMP = 215.7
    real(prec), parameter :: RHO_MIN = 0.3
    real(prec), parameter :: BETA = 5.75
    real(prec), parameter :: EPS = 0.1
    real(prec), parameter :: QR = 70000.0
    real(prec), parameter :: KRSTAR = 0.022
    ...
    nonlinear_arrhenius = (                                               &
    &  MUSTAR                                                             &
    &  * exp((QP / GAS_CONST) * ((1.0 / temperature) - (1.0 / REF_TEMP))) &
    )
    gbs_arrhenius = (                                                     &
    &  KRSTAR                                                             &
    &  * exp((QR / GAS_CONST) * ((1.0 / REF_TEMP) - (1.0 / temperature))) &
    )


    ! determination of the transition density
    norm_radius_max = (                                                &
    &  1.0 + ((((((Z0 - 2.0)**2.0) + (4.0 * C))**0.5) - Z0 + 2.0) / C) &
    )
    transition_dens = ((1.0 / norm_radius_max)**3.0)


    norm_radius_i = ((rel_density / transition_dens)**(1.0 / 3.0))
    do n = 1, nz, 1
      if ( norm_radius_i(n) < 1.0 ) norm_radius_i(n) = 1.0
    end do
    norm_radius_ii = 1.0 * norm_radius_i
    norm_radius_diff = -999999.9

    do while ( maxval(abs(norm_radius_diff)) > 1.0e-1 )

      norm_radius_test = (                  &
      &  (norm_radius_ii**3.0)              &
      &  - (                                &
      &    (Z0 / 4.0)                       &
      &    * ((norm_radius_ii - 1.0)**2.0)  &
      &    * ((2.0 * norm_radius_ii) + 1.0) &
      &  )                                  &
      &  - (                                &
      &    (C / 16.0)                       &
      &    * ((norm_radius_ii - 1.0)**3.0)  &
      &    * ((3.0 * norm_radius_ii) + 1.0) &
      &  )                                  &
      )

      norm_radius_diff = ((norm_radius_test**(1.0 / 3.0)) - norm_radius_i)
      norm_radius_ii = norm_radius_ii + (norm_radius_diff * 1.0e-5)

    end do

    norm_radius_ii = (norm_radius_ii**(1.0 / 3.0))
    coord_number = Z0 * (rel_density / transition_dens)
    zeta = ZETA0 * (coord_number / Z0)

    fraction_not_involved = (                                           & 
    &  1.0                                                              &
    &  - ((Z0 * (norm_radius_ii - 1.0)) / (2.0 * norm_radius_ii))       &
    &  - ((C * ((norm_radius_ii - 1.0)**2.0)) / (4.0 * norm_radius_ii)) &
    )

    average_neck = (                                                                &
    &  ((4.0 * PI * (norm_radius_ii**2.0)) / ((norm_radius_i**2.0) * coord_number)) &
    &  * (1.0 - ((1.0 * zeta) * fraction_not_involved))                             &
    &  * (1.0 - ((1.0 - ((1.0 - zeta) * fraction_not_involved)) / coord_number))    &
    )
    average_neck_0 = ((4.0 * PI * ZETA0) / Z0) * (1.0 - (ZETA0 / Z0))
    do n = 1, nz, 1
      if ( norm_radius_i(n) == 1.0 ) average_neck(n) = average_neck_0
    end do

    spherical_cup = (                                                                &
    &  ((4.0 * PI * (norm_radius_ii**2.0)) / ((norm_radius_ii**2.0) * coord_number)) &
    &  * (1.0 - (1.0 / coord_number))                                                &
    )

    do n = 1, nz, 1
      if ( rel_density(n) < transition_dens ) then

        dilatance_rate(n) = (            &
        &  (rel_density(n) - RHO_MIN)    &
        &  / (transition_dens - RHO_MIN) &
        )**BETA
        gbs_coefficient(n) = (                                            &
        &  0.5                                                         &
        &  * (1.0 - (coord_number(n) / Z0) + (1.0 - (coord_number(n) / Z0))) &
        )

      else

        dilatance_rate(n) = 1.0
        gbs_coefficient(n) = 0.0
    ...
    strain_rate = (                                  &
    &  ((                                            &
    &    ((3.0 * average_neck * spherical_cup)**0.5) &
    &    * rel_density                               &
    &    * (coord_number**2.0)                       &
    &  ) / (                                         &
    &    4.0 * PI * Z0                               &
    ...
    strain_rate = strain_rate * (                                    &
    &  (2.0 * (3.0**0.5) * PI * nonlinear_arrhenius * norm_radius_i) &
    &  / (average_neck * fraction_not_involved * norm_radius_ii)     &
    )
    strain_rate = strain_rate * (                     &
    &  (1.0 - ((1.0 - zeta) * fraction_not_involved)) &
    )
    strain_rate = strain_rate * (                        &
    &  (1.0 - ((1.0 - dilatance_rate) * rearr_fraction)) &
    )
    strain_rate = strain_rate + (                                           &
    &  ((                                                                   &
    &    (EPS * (rel_density**2.0) * coord_number * (3.0 - dilatance_rate)) &
    &    / ((3.0**0.5) * Z0)                                                &
    &  )**ALPHA)                                                            &
    &  * (2.0 * (3.0**0.5) * nonlinear_arrhenius * (1.0 - rearr_fraction))  &
    )
    strain_rate = ((stress**ALPHA) / strain_rate)
    ...
    STOP
  end function tfm_density_salamantin2007
